  ...
  return (
    <header className="sticky top-0 z-50 bg-white/80 dark:bg-[#07090e]/80 backdrop-blur-xl border-b border-gray-200/80 dark:border-gray-800/80 shadow-sm transition-colors duration-300">
      <div className="max-w-[1440px] mx-auto px-2 sm:px-4 md:px-6 py-3 sm:py-4 md:py-5">
        <div className="flex justify-between items-center gap-1 xl:gap-2">
          ...
          <div className="flex items-center gap-1 xl:gap-2 2xl:gap-3 flex-shrink-0">
            ...
            <button
              ...
              className="xl:hidden p-2 sm:p-2.5 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl transition-colors text-gray-600 dark:text-gray-300"
            >
        ...
        {/* Mobile Menu */}
        {mobileMenuOpen && (
          <nav className="xl:hidden mt-3 sm:mt-3 py-3 sm:py-4 border-t border-gray-200 dark:border-gray-800 space-y-2">
            {navItems.map((item) => (
              <button
                key={item.id}
                onClick={() => scrollToSection(item.id)}
            ...
            {!isUser ? (
              ...
              <div className="grid grid-cols-2 gap-3 pt-2">
                <Link
                  href={userRole === 'admin' ? '/admin' : '/account'}
                  className="w-full text-center py-3 text-sm font-bold text-white bg-gradient-to-r from-emerald-600 to-green-600 rounded-xl shadow-md flex items-center justify-center gap-2"
                >
                ...
                <button
                  onClick={handleLogout}
                  disabled={isLoggingOut}
                  className="w-full text-center py-3 text-sm font-bold text-white bg-gradient-to-r from-red-600 to-rose-600 rounded-xl shadow-md flex items-center justify-center gap-2"
                >
